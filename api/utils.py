...
class RecommenderModel(nn.Module):
    def __init__(self, industry_size, product_name_size, product_category_size, hidden_dim=256, padding_value=-1):
    ...
    def forward(self, industry, product_name, product_category, product_is_implemented, new_size=None):
        if new_size is not None and new_size > self.product_name_size:
...
class RecommenderDataset(Dataset):
    def __init__(self, data, industry_map, product_name_map, product_category_map, device="cpu"):
        ...
        for x in data:
            indices = torch.tensor([product_name_map[y] for y in x["Output"]["Names"]]).long()
            values = torch.tensor(x["Output"]["Weights"]).float()

            target = torch.zeros(product_name_len)

            target[indices[indices < product_name_len]] = values
